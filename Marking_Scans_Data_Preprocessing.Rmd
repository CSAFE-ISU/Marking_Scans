...
for(i in 1:nrow(df_1)){
  ...
  df_1$x3p[[i]]$mask[which(is.na(df_1$x3p[[i]]$surface.matrix) & df_1$x3p[[i]]$mask == "#CD7F32FF")] <- "#000000"  #Replace Mask elements corresponding to missing value locations of surface.matrix
...
for(i in 1:nrow(df_2)){
  ...
  df_2$x3p[[i]]$mask[which(is.na(df_2$x3p[[i]]$surface.matrix) & df_2$x3p[[i]]$mask == "#CD7F32FF")] <- "#000000"
  df_2$x3p[[i]]$mask <- t(df_2$x3p[[i]]$mask)
}
