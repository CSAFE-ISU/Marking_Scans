...
for(i in 1:nrow(df_1)){
  ...
  df_1$x3p[[i]] <- x3p_add_annotation(df_1$x3p[[i]], color = "#000000FF", annotation = "Missing Value") #Create New Annotation and Color code corresponding to that annotation
  df_1$x3p[[i]]$mask <- t(df$x3p[[i]]$mask)         #Transpose mask matrix so the j rows and k columns correspond to the j rows and k columns of the surface matrix                                                    
  
  for(j in 1:nrow(df_1$x3p[[i]]$surface.matrix)){
    for(k in 1:ncol(df_1$x3p[[i]]$surface.matrix)){
      
      if(is.na(df_1$x3p[[i]]$surface.matrix[j,k])){ #detect if NA or not
        
        df_1$x3p[[i]]$mask[j,k] <- "#000000FF" #if Na replace mask color code with the new missing value color code
  ...
  df_1$x3p[[i]]$mask <- t(df$x3p[[i]]$mask) #Transpose the new matrix back and store as new mask 
