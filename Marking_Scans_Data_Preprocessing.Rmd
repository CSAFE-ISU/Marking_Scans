...
for(i in 1:nrow(df_1)){
  ...
  df_1$x3p[[i]] <- x3p_add_annotation(df_1$x3p[[i]], color = "#000000FF", annotation = "Missing Value") 
  df_1$x3p[[i]]$mask <- t(df$x3p[[i]]$mask)                                                             
  
  for(j in 1:nrow(df_1$x3p[[i]]$surface.matrix)){
    for(k in 1:ncol(df_1$x3p[[i]]$surface.matrix)){
      
      if(is.na(df_1$x3p[[i]]$surface.matrix[j,k])){
        
        df_1$x3p[[i]]$mask[j,k] <- "#000000FF"
  ...
  df_1$x3p[[i]]$mask <- t(df$x3p[[i]]$mask)
} #This function does not work properly, do not run 



for(i in 1:nrow(df_1)){
  
  df_1$x3p[[i]] <- x3p_add_annotation(df_1$x3p[[i]], color = "#000000FF", annotation = "Missing Value") # Add missing values color code and missing values annotation
  df_1$x3p[[i]]$mask <- t(df_1$x3p[[i]]$mask) #Transpose matrix to match surface matrix 
  df_1$x3p[[i]]$mask <- ifelse(is.na(df_1$x3p[[i]]$surface.matrix),"#000000FF", df_1$x3p[[i]]$mask) #replace missing value masks with the new annotation
  df_1$x3p[[i]]$mask <- t(df_1$x3p[[i]]$mask) #Transpose again so mask is back to original dimensions
}
...
for(i in 1:nrow(df_2)){
  ...
  df_2$x3p[[i]] <- x3p_add_annotation(df_2$x3p[[i]], color = "#000000FF", annotation = "Missing Value")
  df_2$x3p[[i]]$mask <- t(df_2$x3p[[i]]$mask)
  
  for(j in 1:nrow(df_2$x3p[[i]]$surface.matrix)){
    for(k in 1:ncol(df_2$x3p[[i]]$surface.matrix)){
      
      if(is.na(df_2$x3p[[i]]$surface.matrix[j,k])){
        
        df_2$x3p[[i]]$mask[j,k] <- "#000000FF"
...
for(i in 1:nrow(df_2)){
  ...
  df_2$x3p[[i]] <- x3p_add_annotation(df_2$x3p[[i]], color = "#000000FF", annotation = "Missing Value")
  df_2$x3p[[i]]$mask <- t(df_2$x3p[[i]]$mask)
  df_2$x3p[[i]]$mask <- ifelse(is.na(df_2$x3p[[i]]$surface.matrix),"#000000FF", df_2$x3p[[i]]$mask)
  df_2$x3p[[i]]$mask <- t(df_2$x3p[[i]]$mask)
}
