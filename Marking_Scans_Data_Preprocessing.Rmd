...
df_2 <- df_1 # For 256x256 processing
  ...
  for(j in 1:nrow(df_1$x3p[[i]]$surface.matrix)){           
    for(k in 1:ncol(df_1$x3p[[i]]$surface.matrix)){
      ...
      if(is.na(pad_surf[j,k]) & #Find NA values
       ...
       any(!is.na(c(pad_surf[j-1, k-1], pad_surf[j-1, k], pad_surf[j-1, k+1], pad_surf[j, k-1], pad_surf[j+1, k-1], pad_surf[j+1, k], pad_surf[j+1, k+1])))){ #Check if neighbor elements are NA or Not
        
        
        pad_surf[j,k] <- mean(c(pad_surf[j-1, k-1], pad_surf[j-1, k], pad_surf[j-1, k+1], pad_surf[j, k-1], pad_surf[j+1, k-1], pad_surf[j+1, k], pad_surf[j+1, k+1]), na.rm = TRUE) #Replace missing element's with average of missing element's neighbors
        
       } else{pad_surf[j,k]} #If not NA do nothing
      
    }
  }
  
  df_1$x3p[[i]]$surface.matrix <- pad_surf[-c(min(seq(nrow(pad_surf))), max(seq(nrow(pad_surf)))), -c(min(seq(ncol(pad_surf))), max(seq(ncol(pad_surf))))] #Remove temporary zero padding and re-store the new surface.matrix
}
...
for(i in 1:nrow(df_2)){
  ...
  pad_surf <- rbind(0, cbind(0, df_2$x3p[[i]]$surface.matrix, 0), 0) #Create temporary padded surface.matrix so window does not go out of bounds.
                                                                   #Using zeros as padding does not affect the calculations
  ...
  df_2$x3p[[i]]$surface.matrix <- pad_surf[-c(min(seq(nrow(pad_surf))), max(seq(nrow(pad_surf)))), -c(min(seq(ncol(pad_surf))), max(seq(ncol(pad_surf))))] #Remove temporary zero padding and re-store the new surface.matrix
}
