...
for(i in 1:nrow(df_1)){
  df_1$x3p[[i]] <- x3p_add_annotation(df_1$x3p[[i]], color = "#000000FF", annotation = "Missing Value") #Create new annotation and corresponding color code for Missing Values
  df_1$x3p[[i]]$mask <- t(df_1$x3p[[i]]$mask)                                                           #Transpose mask so dimensions correspond to surface.matrix dimensions
  df_1$x3p[[i]]$mask[which(is.na(df_1$x3p[[i]]$surface.matrix) & df_1$x3p[[i]]$mask == "#CD7F32FF")] <- "#000000FF"  #Replace Mask elements corresponding to missing value locations of surface.matrix
...
for(i in 1:nrow(df_2)){
  ...
  df_2$x3p[[i]]$mask[which(is.na(df_2$x3p[[i]]$surface.matrix) & df_2$x3p[[i]]$mask == "#CD7F32FF")] <- "#000000FF"
  df_2$x3p[[i]]$mask <- t(df_2$x3p[[i]]$mask)
}
